...
export class AddToCartComponent extends Component {
   ...
   */
  handleClick(event) {
    // "Get Notified" state is intentionally static: no submit, no cart add, no animation.
    if (this.refs.addToCartButton.dataset.notify === 'true') {
...
class ProductFormComponent extends Component {
  ...
  handleSubmit(event) {
    ...
    // "Get Notified" (sold-out, static) buttons must never add to cart.
    if (this.refs.addToCartButtonContainer?.refs.addToCartButton?.dataset.notify === 'true') return;
  ...
  #onVariantUpdate = async (event) => {
    ...
    const newButtonIsNotify = newAddToCartButton?.getAttribute('data-notify') === 'true';
    const resourceUnavailable = event.detail.resource == null || event.detail.resource.available == false;

    // Morph the button's children (text/icon) from the new server render.
    if (newAddToCartButton && currentAddToCartButton) {
      morph(currentAddToCartButton, newAddToCartButton);
    }

    // The button morph above is childrenOnly, so it does not sync the button's own
    // attributes. Drive the notify/disabled state directly here so add-to-cart keeps
    // working across variant changes. A "Get Notified" button stays enabled but inert.
    if (currentAddToCartButton) {
      if (newButtonIsNotify) {
        currentAddToCartButton.dataset.notify = 'true';
        currentAddToCartButtonContainer.enable();
      } else {
        delete currentAddToCartButton.dataset.notify;
